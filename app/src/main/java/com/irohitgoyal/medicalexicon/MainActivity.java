...
public class MainActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (errorDB.isEmpty()) {
			...
			if ((StaticData.isFirstRun.booleanValue()) && (freeSpace.longValue() < 100L)) {
				...
				builder.setTitle("ABC").setMessage("XYZ").setIcon(android.R.drawable.stat_sys_warning)
						.setCancelable(false).setNegativeButton("DEF", new DialogInterface.OnClickListener() {
	...
	public void dataBaseError(String paramString) {
		if (paramString.equals("write failed: ENOSPC (No space left on device)")) {
			...
			localBuilder.setTitle(getString(R.string.not_have_free_space)).setMessage(getString(R.string.message_100mb))
					.setIcon(android.R.drawable.stat_sys_warning).setCancelable(false)
